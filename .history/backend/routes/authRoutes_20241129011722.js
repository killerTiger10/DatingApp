...
const { generateTokens } = require("./auth"); // Assuming generateTokens is in a separate file
...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    const saltRounds = await bcrypt.genSalt(10); // Adjust rounds as needed
    ...
    console.error(err);
    // Handle specific errors from User.save() if needed
    if (err.code === 11000) {
    ...
    res.status(500).json({ error: "Internal server error" }); // Generic error for unexpected errors
  }
});
// router.post("/register", async (req, res) => {
//   const { username, email, password } = req.body;
